...
void generate_permutations(Graph* gm, Graph* gf, int* current_mapping, int * originalnodes, int* nodes, int original_delta, int k, int depth, int* best_delta, int * best_combi, int index, int* improvements, const char* out_path, int max_node) {
    if (depth == k) {
        ...
        #pragma omp critical 
        {	
            if (new_delta -original_delta>*best_delta ) {
		*best_delta=new_delta-original_delta;
		*best_combi=index;
	    }
	}
    ...
    for (int i = depth; i < k; i++) {
        swap(nodes, depth, i);
        generate_permutations(gm, gf, current_mapping, originalnodes, nodes, k, depth + 1, best_delta, best_combi, index, improvements, out_path, max_node);
...
void process_permutation(Graph* gm, Graph* gf, int* current_mapping, int* originalnodes, int* nodes,int original_delta, int k, int* best_delta,int * best_combi, int index, int* improvements, const char* out_path, int max_node) {
	
    // Use generate_permutations to test all permutations of k nodes
    generate_permutations(gm, gf, current_mapping, originalnodes, nodes, original_delta,k, 0, best_delta, best_combi, index, improvements, out_path, max_node);
}
...
void generate_combinations_parallel(int n, int k, Graph* gm, Graph* gf, int* current_mapping, 
	...
	int best_combi=-1;
        ...
        #pragma omp parallel for shared(max_delta,best_combi) schedule(dynamic)
        for (int i = 0; i < batch_count; i++) {
            process_permutation(gm, gf, current_mapping, combinations[i].originalcombo,
                              combinations[i].combo,combinations[i].delta, k, &max_delta,
			      &best_combi,i, 
                              improvements, out_path, max_node);
        }
	if (max_delta>0) {
            // Apply the new permutation of nodes to temp_mapping
            int oldcore=calculate_alignment_score(gm, gf, current_mapping);
            int* temp_mapping = malloc(sizeof(int) * (max_node + 1));
            memcpy(temp_mapping, current_mapping, sizeof(int) * (max_node + 1));

            // Apply the new permutation of nodes to temp_mapping
            for (int i = 0; i < k; i++) {
                temp_mapping[combinations[best_combi].combo[i]] = 
			combinations[best_combi].originalcombo[i+k];
            }
