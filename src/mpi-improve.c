...
void random_swap_k_vertices(int* mapping, int n, int k,int seed,int * num_changed, int * changed_vertices) {
...
int* optimize_mapping(Graph* gm, Graph* gf, int* initial_mapping,
    ...
    int* changed_vertices = malloc(sizeof(int) * (max_node + 1));
	...
	// Check if it's time to sync
        ...
        if (difftime(current_time, last_sync_time) >= SYNC_INTERVAL) {
	      ...
	      free(benchmark_mapping);
              ...
              LOG_INFO("Average execution time for one iteration is  %f", difftime(current_time,start_time)*1.0/iter);

        }

    // Optimization loop

    random_swap_k_vertices(current_mapping, max_node, 3+rank, rank, &num_changed, changed_vertices);
    current_score = calculate_alignment_score(gm, gf, current_mapping);
    for (int node_m1_idx=0; node_m1_idx<num_changed; node_m1_idx++) {
	...
	*/
	...
	node_m1=changed_vertices[node_m1_idx];
	    ...
	    for (int j=0;j<num_changed;j++) {
		    if (changed_vertices[j]==node_m2 && node_m1>node_m2) {
			    skip=1;
			    break;
		    }
	    }
    ...
    if (pass >2 && improvements-last==0 ) { 
		...
		int shuffle=3;
                ...
                random_swap_k_vertices(current_mapping, max_node, shuffle+rank, rank, &num_changed, changed_vertices);
                current_score = calculate_alignment_score(gm, gf, current_mapping);
		improvements=0;
		last=0;
		pass=0;
