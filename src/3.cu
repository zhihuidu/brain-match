...
#define BATCH_SIZE  170000000  // Adjust based on GPU memory
...
__global__ void calculateThreeNodeSwapKernel(
    ...
    int global_idx = batch_start + idx;
    int n = num_nodes;
    ...
    for (int p = 1; p < 6; p++) {  // Start from 1 as 0 is original mapping
        ...
        // Calculate delta for each node with rest of graph
        for (int i = 1; i <= num_nodes; i++) {
            ...
            // Similar calculations for m2 and m3...
            // (Adding complete edge calculations for all nodes)
        }
...
int* optimize_mapping(Graph* gm, Graph* gf, int* initial_mapping, const char* out_path) {
    ...
    int total_triplets = (max_node * (max_node - 1) * (max_node - 2)) / 6;
    ...
    while (true) {
        ...
        while (batch_start < total_triplets) {
            int current_batch_size = min(batch_size, total_triplets - batch_start);
