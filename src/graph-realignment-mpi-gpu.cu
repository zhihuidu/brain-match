...
int* optimize_mapping(Graph* gm, Graph* gf, int* initial_mapping, const char* out_path) {
    ...
    while (true) {
        ...
        if (difftime(current_time, last_sync_time) >= SYNC_INTERVAL) {
            LOG_INFO("Process %d, synchronizing with best score %d", rank, best_score);
        ...
        if (pass > 2 && improvements - last == 0) {
            LOG_INFO("Process %d restarting optimization with perturbation", rank);
            memcpy(current_mapping, best_mapping, (max_node + 1) * sizeof(int));
            random_swap_k_vertices(current_mapping, max_node, 3 + rank, rank);
            current_score = calculate_alignment_score(gm, gf, current_mapping);
            improvements = 0;
            last = 0;
            pass = 0;
	...
	if (iter++ %10==0) {
            ...
            LOG_INFO("Average time for once brute-force search is  %f", (current_time-start_time)/iter);
            LOG_INFO("Current score is %d  best score is %d of rank %d", current_score, best_score, rank);
	}
    }
