...
int calculate_swap_delta(Graph* gm, Graph* gf, int* mapping, int node_m1, int node_m2) {
    ...
    for (int i = 0; i < gm->edges[node_m1].count; i++) {
        ...
        int old_weight = MIN(weight_m, get_weight(gf, node_f1, dst_f));
        // Add new contribution
        int new_weight = MIN(weight_m, get_weight(gf, node_f2, dst_f));
    ...
    for (int i = 0; i < gm->reverse_edges[node_m1].count; i++) {
        ...
        // Remove old contribution
        int old_weight = MIN(weight_m, get_weight(gf, src_f, node_f1));
        // Add new contribution
        int new_weight = MIN(weight_m, get_weight(gf, src_f, node_f2));
    ...
    for (int i = 0; i < gm->edges[node_m2].count; i++) {
        ...
        // Remove old contribution
        int old_weight = MIN(weight_m, get_weight(gf, node_f2, dst_f));
        // Add new contribution
        int new_weight = MIN(weight_m, get_weight(gf, node_f1, dst_f));
    ...
    for (int i = 0; i < gm->reverse_edges[node_m2].count; i++) {
        ...
        // Remove old contribution
        int old_weight = MIN(weight_m, get_weight(gf, src_f, node_f2));
        // Add new contribution
        int new_weight = MIN(weight_m, get_weight(gf, src_f, node_f1));
    ...
    // From m1 to m2
    int m1_to_m2 = get_weight(gm, node_m1, node_m2);
    if (m1_to_m2 > 0) {
        int old_weight = MIN(m1_to_m2, get_weight(gf, node_f1, node_f2));
        int new_weight = MIN(m1_to_m2, get_weight(gf, node_f2, node_f1));
        delta += new_weight - old_weight;
    }
    
    // From m2 to m1
    int m2_to_m1 = get_weight(gm, node_m2, node_m1);
    if (m2_to_m1 > 0) {
        int old_weight = MIN(m2_to_m1, get_weight(gf, node_f2, node_f1));
        int new_weight = MIN(m2_to_m1, get_weight(gf, node_f1, node_f2));
        delta += new_weight - old_weight;
    }
    ...
    for (int i = 0; i < gm->node_count; i++) {
        ...
        if (outer_node_count % UPDATE_INTERVAL == 0) {
            ...
            LOG_INFO("  - Processed: %s/%s nodes (%.1f%%)",
                    ...
                    format_number(gm->node_count),
                    (double)outer_node_count/gm->node_count * 100);
            ...
            LOG_INFO("  - Estimated time remaining: %.1f minutes",
                    (gm->node_count - outer_node_count) / nodes_per_sec / 60);
        }
...
Graph* load_graph_from_csv(const char* filename) {
    ...
    LOG_INFO("  - Nodes: %s", format_number(graph->node_count));
